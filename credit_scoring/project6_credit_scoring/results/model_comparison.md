...
**trước khi sửa** của bảng ở mục 4, giữ lại để đối chiếu, và không sinh ra được từ code hiện tại.
...
`monthly_income` gộp hai cặp, nên chấm là trúng một phần.

Ngoài ra khối này có **ba kết luận tôi viết ra rồi phải rút lại**, và cả ba đáng nói hơn bản
...
số, không phải xấp xỉ. Con số 1,88 là suppression, và gần như toàn bộ mức nhảy đến từ
`monthly_income`.
...
là bằng chứng cho tính không cộng được của Gini, không phải cho một sai sót.

Điều rút ra: **toàn bộ bảng đóng góp biên xáo trộn mạnh trong khi Gini tổng đứng yên.** "Biến
...
hình (`max_depth=3`, `lr=0,10`, 250 vòng), nên ba bước chỉ đổi đầu vào, không lẫn "đổi cấu
hình".
...
### Lỗi cài đặt thứ nhất: early stopping khuếch đại nhiễu
...
thiên lệch mà trước đó chỉ khai được, không tách được. Và bảng test dùng đúng số vòng của bảng
CV, nên hai bảng đọc chéo nhau được. Cái giá: số vòng thành một siêu tham số nữa, khai trước theo
quy tắc `learning_rate × n_estimators ≈ 25`, không dò tìm.
...
### Lỗi cài đặt thứ hai: cây được nhìn nhãn của fold đánh giá

Ba model cây ban đầu nhận thẳng bảng `features_woe_mono`, tức cột WOE tính trên trọn tập train.
Với M3 và M4 thì không sao vì chúng chạy trên biến gốc, nhưng M2 và cây `depth=1` chạy trên chính
cột WOE đó, nên fold đánh giá đã góp nhãn của nó vào việc tạo feature cho chính nó. Scorecard
tránh chuyện này từ khối 3 bằng cách tính lại WOE trong từng fold; cây thì không, nên phép so
sánh nghiêng về phía cây.

Đo bằng logistic trên cùng bộ fold: WOE tính trong fold cho 0,71507, WOE toàn train cho 0,71570,
chênh 0,00064, trong đó Laplace của `woe_of` chỉ đáng 0,00001. Nhưng mức ảnh hưởng khác hẳn giữa
hai lớp model, và chỗ này mới đáng ghi: logistic dùng trị số WOE làm log-odds nên mọi xê dịch
do nhãn gây ra đi thẳng vào dự báo, còn cây chỉ dùng WOE làm thứ tự rồi tự cắt lại. Đo M2 trước
và sau khi sửa: 0,72492 xuống 0,72481, tức 0,00011, nhỏ hơn con số của logistic sáu lần.

Vì hai lớp model hưởng rò rỉ khác nhau nên không được lấy số của bên này suy ra cho bên kia, và
cũng không được sửa bằng cách đổi mốc so sánh: mốc phải sạch, cả hai vế phải sạch. Quyết định là
tính lại WOE trong từng fold cho **mọi** model chạy trên cột WOE. Sau khi sửa, tự do hình dạng là
+0,00054 với `t` = 1,27, vẫn dương và vẫn không phân biệt được với 0, gần như đúng con số trước
khi sửa.

`tree_model.woe_design` dựng ma trận WOE riêng cho từng fold, dùng lại `cv_check._design` để
scorecard và cây chắc chắn đi qua cùng một đường tính. Notebook có một `assert` bắt baseline
logistic của mục 4 phải trùng khít Gini CV của scorecard; nó trùng tới 0,00e+00.

### Bước scorecard → M2 đổi hai thứ cùng lúc
...
| XGB `depth=1` trên đúng cột WOE đó | 0,71561 | tự do hình dạng = +0,0005, t = 1,27, KTC [−0,0006; +0,0017] |
| XGB `depth=4` trên đúng cột WOE đó | 0,72444 | tương tác = +0,0088, t = 8,1 |

Tự do hình dạng không phân biệt được với 0, còn tương tác thì lớn hơn hẳn. Lý do cơ học và đáng
...
2000 cho 0,71569. Ba con số này đo trước khi sửa lỗi cài đặt thứ hai nên mức của chúng ứng với
bản cũ; điều chúng dùng để kết luận là 1200 vòng đã hội tụ, và điều đó không phụ thuộc mức.
...
Từ đó tôi lấy 0,002 làm ngưỡng phân biệt được với nhiễu. Con số này **không suy ra được** từ 0,00110; nó là
biên độ làm tròn lên gần gấp đôi, chọn cho an toàn.

Phép đo nhiễu này chạy tại đúng cấu hình đã thắng vòng chọn siêu tham số, nên nó đo nhiễu của
**một lần fit**, không đo nhiễu của cả quy trình "chọn rồi báo cáo". Khi ngưỡng 0,002 và kiểm
...
| XGB `depth=1` trên cùng cột WOE | 0,71561 | +0,0005 | 1,27 | tự do hình dạng, không phân biệt được với 0 |
| M2 XGB `depth=3` trên cùng cột WOE | 0,72481 | +0,0097 | 14,1 | tương tác |
| M3 XGB trên biến gốc | 0,72935 | +0,0045 | 8,8 | chỗ cắt bin |
| M4 XGB trên tất cả | 0,73280 | +0,0035 | 8,26 | biến scorecard không dùng |

Dòng M2 so với dòng ngay trên nó là +0,0097; phần thuần tương tác, tức so với cây `depth=4` trên
cùng cột WOE, là +0,0088. Hai con số chênh nhau đúng phần tự do hình dạng.
...
- **Tương tác là kênh lớn nhất, +0,0088**; tự do hình dạng không phân biệt được với 0.
- Chỗ cắt bin và biến bị loại mỗi kênh đáng khoảng +0,004, cả hai đều đo được.
...
|---|---|---|
| `max_depth=4`, `lr=0,10`, 250 vòng | +0,00370 (t = 6,18) | +0,00439 (t = 8,99) |
| `max_depth=5`, `lr=0,05`, 500 vòng | +0,00396 (t = 6,90) | +0,00409 (t = 10,31) |

Bốn con số nằm trong khoảng hẹp 0,0037 đến 0,0044 và cả bốn đều có `t` lớn, nên **độ lớn của hai
kênh là kết luận đo được**. Thứ không kết luận được chỉ là kênh nào lớn hơn: hai kênh gần bằng
nhau và chênh lệch giữa chúng (0,0007 và 0,0001) nhỏ hơn mức phân biệt được.

Phát biểu đúng: **phần vượt của cây chủ yếu là tương tác (+0,0088), phần còn lại chia gần đều cho
...
của early stopping, không phải một quy luật.
...
0,0088), nên kết luận đó vẫn đứng. Dự đoán B1 và B2 đều trúng, và tổng phần vượt nằm xa dưới
...
Bảng này đã sửa hai lần và cả hai lần đều theo hướng làm kết luận chắc hơn. Bản đầu gọi
`LogisticRegression(C=1e12, max_iter=2000)`, tức đúng cấu hình không hội tụ đã làm hỏng `fit_logit`
ở khối 3; bản thứ hai còn lấy cột WOE tính sẵn trên trọn train nên riêng nhánh "+ cột WOE" được
nhìn nhãn của fold đánh giá. Qua cả hai lần sửa, dòng chín dummy đi từ +0,00207 xuống +0,00200 và
dòng một hệ số đi từ +0,00024 (t = 2,36) xuống +0,00013 (t = 1,26). Kết luận không đổi, nhưng mỗi
bản lỗi đều đẩy dòng một hệ số lên gần mức phân biệt được với 0 hơn thực tế.

| cách đưa `open_credit_lines` vào model | đóng góp | t |
|---|---|---|
| biến gốc, cộng tính tự chọn hình dạng (`depth=1`) | +0,0028 | 3,67 |
| cột WOE khối 2, cộng tính (`depth=1`) | +0,0023 | 4,68 |
| cột WOE khối 2, logistic với 9 cột dummy | +0,0020 | 3,95 |
| cột WOE khối 2, logistic với 1 hệ số, cùng phép đo với dòng trên | +0,00013 | 1,26 |
...
Vì lý do đó so sánh chính ở mục 3 dùng M3 thay vì M4: hai bên phải cùng bộ biến, nếu không
...
Giá trị SHAP interaction lấy thẳng từ XGBoost (`pred_interactions=True`) thay vì qua gói
...
được**, và nguyên nhân mới là chỗ đáng nói. Ba lần chạy khác nhau cho **ba cặp đứng đầu khác
...
thô để nhiễu không lật được, và mệnh đề đó đúng ở cả ba lần chạy. Đây là lỗi thiết kế dự đoán,
không phải lỗi đo.
...
Tương tác chiếm 28,7% tổng |SHAP| nhưng kênh tương tác chỉ quy ra +0,0088 Gini. Hai con số
không mâu thuẫn, và phải nói kèm hai điều. Thứ nhất chúng **của hai model khác nhau**: 28,7% đo
trên M4 (biến thô, thêm hai biến), còn +0,0088 đo trên M2 (cột WOE). Thứ hai, ngay cả trong cùng
một model thì phần lớn tương tác là tinh chỉnh quanh một hiệu ứng chính rất mạnh, không tạo ra
thứ tự xếp hạng mới.
...
**Thứ tự giữa ba model cây thì không kết luận được**, và ở đây có một ví dụ trực tiếp: CV nói M3
(0,72935) hơn M2 (0,72481) một cách rõ ràng (t = 8,8), còn trên test thì hai model gần như bằng
nhau và M2 nhỉnh hơn (0,7139 so với 0,7137). Chênh lệch đó nằm sâu trong khoảng tin cậy của chính
Gini trên test.

Con số nên tin vẫn là CV, không phải test: test chỉ có 1.504 ca dương nên khoảng tin cậy 95%
...
PD dự báo trung bình của cả bốn model đều quanh 6,56 đến 6,61% so với bad rate thực 6,684%.
...
- Cấu hình thắng hơn cấu hình nhì 0,00037 ở M2, 0,00121 ở M3, 0,00027 ở M4, tức cùng bậc với mức
...
- Số vòng boosting khai trước theo quy tắc `lr × n_estimators ≈ 25`, không dò tìm, nên không
...
- Điểm cắt bin và nhóm PAVA vẫn học trên toàn bộ train rồi dùng lại trong mọi fold; chỉ giá trị
  WOE mới được tính lại trong fold. Riêng nhóm PAVA sinh ra **từ WOE, tức từ nhãn**, nên lý lẽ đã
  dùng ở khối 3 để tha cho điểm cắt `NTILE` (phân vị của biến, không nhìn nhãn) không mở rộng sang
  đây được. Sau khi sửa lỗi cài đặt thứ hai thì thiên lệch này áp cho cả scorecard lẫn M2, vì
  hai bên dùng chung bộ bin; nó không còn nghiêng về bên nào.
